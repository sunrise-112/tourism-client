...
const STATUS = {
  confirmed: { bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-200", dot: "bg-emerald-400", label: "Confirmed" },
  ...
  completed: { bg: "bg-stone-50",   text: "text-stone-500",   border: "border-stone-200",   dot: "bg-stone-400",   label: "Completed" },
};
...
const Sk = ({ className }) => <div className={`animate-pulse bg-stone-100 rounded-xl ${className}`} />;
...
const MyBookings = () => {
  ...
  return (
    <div className="min-h-screen bg-stone-50 p-6 md:p-8" style={{ fontFamily: "'DM Sans', sans-serif" }}>

      {/* Page header */}
      <div className="mb-8">
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-amber-500 mb-1">History</p>
        <h1 className="text-3xl font-black text-stone-800" style={{ fontFamily: "'Playfair Display', serif" }}>
          My Bookings
        </h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <div className="flex items-center gap-1 bg-white border border-stone-200 rounded-xl p-1 overflow-x-auto">
          {FILTERS.map((f) => (
            ...
            >
              {f}
              <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-full ${filter === f ? "bg-amber-300/60 text-amber-900" : "bg-stone-100 text-stone-400"}`}>
                {counts[f]}
              </span>
            </button>
          ))}
      ...
      {loading ? (
        <div className="space-y-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-white rounded-2xl border border-stone-100 flex overflow-hidden">
        ...
        <div className="bg-white rounded-2xl border border-stone-100 py-20 text-center">
          ...
          <p className="text-sm text-stone-400 mb-6">
            {search || filter !== "all" ? "Try adjusting your filters" : "You haven't booked any tours yet"}
          </p>
          <Link to="/tours"
              ...
              <div className="flex flex-col sm:flex-row">
                ...
                <div className="sm:w-44 h-44 sm:h-auto shrink-0 relative overflow-hidden bg-stone-100">
                  {booking.tour?.cover_image ? (
                    <img src={booking.tour.cover_image} alt={booking.tour?.title}
                  ...
                  {booking.tour?.duration_days && (
                    <div className="absolute bottom-2 left-2 bg-black/50 text-white text-[10px] px-2 py-1 rounded-full backdrop-blur-sm font-medium flex items-center gap-1">
                      <i className="fa fa-clock text-[9px]" /> {booking.tour.duration_days}d
                    </div>
                  )}
                </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 pt-4 mt-4 border-t border-stone-100">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        ...
                        <p className="text-lg font-black text-amber-600">${Number(booking.total_price || 0).toLocaleString()}</p>
                      </div>
                      <Link to={`/tours/${booking.tour?.id}`}
