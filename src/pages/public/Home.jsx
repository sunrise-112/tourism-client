...
const defaultCategories = [
  { labelKey: "home.categories.adventure", icon: "fa-mountain", bg: "#FFF4ED", color: "#C2500A" },
  { labelKey: "home.categories.beach", icon: "fa-umbrella-beach", bg: "#EDF7FF", color: "#0A6EC2" },
  { labelKey: "home.categories.cultural", icon: "fa-landmark", bg: "#F5F0FF", color: "#6B21A8" },
  { labelKey: "home.categories.wildlife", icon: "fa-paw", bg: "#EDFFF4", color: "#15803D" },
  { labelKey: "home.categories.city", icon: "fa-city", bg: "#EFF6FF", color: "#1D4ED8" },
  { labelKey: "home.categories.wellness", icon: "fa-spa", bg: "#FFF0F6", color: "#BE185D" },
];

// ─── Why Choose Us ────────────────────────────────────────────
const whyChooseUs = [
  { icon: "fa-shield-alt", titleKey: "home.why.safe", descKey: "home.why.safeDesc" },
  { icon: "fa-tag", titleKey: "home.why.price", descKey: "home.why.priceDesc" },
  { icon: "fa-headset", titleKey: "home.why.support", descKey: "home.why.supportDesc" },
  { icon: "fa-route", titleKey: "home.why.custom", descKey: "home.why.customDesc" },
];
...
const TourCard = ({ tour, t, index = 0 }) => {
  const typeLabelKey = tour.type ? `home.tourCard.type.${tour.type.toLowerCase()}` : null;
  const typeLabel = typeLabelKey && t(typeLabelKey) !== typeLabelKey ? t(typeLabelKey) : tour.type || "";
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-stone-100 mt-auto">
          <div>
            <span className="text-xs text-stone-400 block mb-0.5">{t("home.tourCard.from")}</span>
            <p className="text-2xl font-black text-amber-600 tracking-tight">${tour.price}</p>
          </div>
          <span className="flex items-center gap-2 text-sm font-semibold text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 px-5 py-2.5 rounded-xl transition-all duration-300 group-hover:gap-3">
            {t("home.tourCard.explore")} <i className="fa fa-arrow-right text-xs transition-transform group-hover:translate-x-1" />
          </span>
        </div>
      </div>
    </Link>
...
const SectionHeader = ({ eyebrowKey, titleKey, subtitleKey, right, t }) => (
  <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
    <div className="max-w-2xl">
      ...
      {subtitleKey && (
        <p className="text-stone-400 text-base mt-3 leading-relaxed">{t(subtitleKey)}</p>
      )}
    </div>
...
const Home = () => {
  ...
  useEffect(() => {
    carouselIntervalRef.current = setInterval(() => {
      setCurrentImageIndex((prev) => (prev === HERO_BACKGROUND_IMAGES.length - 1 ? 0 : prev + 1));
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [featuredRes, dealsRes, toursRes, excursionsRes, activitiesRes] = await Promise.all([
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (search.trim()) window.location.href = `/tours?q=${encodeURIComponent(search)}`;
  };
    ...
    <div className="min-h-screen bg-stone-50" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section ref={heroRef} className="relative h-[100dvh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center pt-20">
          <AnimatedSection>
            <div className="inline-flex items-center gap-2 bg-white/10 text-amber-200 text-xs font-semibold px-5 py-2.5 rounded-full border border-white/20 mb-8 backdrop-blur-md shadow-lg shadow-black/10 hover:bg-white/15 transition-colors cursor-default">
              <i className="fa fa-globe-africa animate-spin-slow" style={{ animationDuration: '8s' }} />
              {t("home.hero.eyebrow")}
            </div>
          </AnimatedSection>
          ...
          <AnimatedSection delay={200}>
            ...
            >
              ...
              <span
                ...
                style={{
                  backgroundImage: "linear-gradient(135deg, #FCD34D, #F59E0B, #FB923C, #FCD34D)",
          ...
          <AnimatedSection delay={600}>
            ...
            >
              ...
              >
                {t("home.hero.searchButton")} <i className="fa fa-arrow-right text-xs" />
              </button>
            </form>
          </AnimatedSection>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-white/60 z-10 animate-bounce">
          <span className="text-[10px] tracking-[0.3em] uppercase font-medium">{t("home.hero.scroll")}</span>
        ...
        <div className="max-w-6xl mx-auto px-6 py-14 relative">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-4 divide-x divide-amber-400/30">
            {translatedStats.map((s, i) => (
              <div key={i} className={`${i % 2 === 1 ? 'md:border-r md:border-l' : ''} md:border-amber-400/30`}>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        ...
        <div className="flex items-center justify-center gap-3 mt-10">
          {!showMore ? (
            <button
              onClick={() => { fetchCategories(30); setShowMore(true); }}
            ...
            <button
              onClick={() => { fetchCategories(); setShowMore(false); }}
              className="flex items-center gap-2 px-8 py-3 rounded-full border border-stone-200 bg-white text-xs font-bold tracking-wider uppercase text-stone-500 hover:bg-stone-50 hover:border-stone-300 hover:text-stone-700 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 cursor-pointer"
            >
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-24 text-stone-300">
              ...
              <p className="text-stone-400 text-lg">{t("home.featured.empty")}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      ...
      {!loading && (
        <>
          {renderSection("home.toursSection.title", tours, "/tours?type=tour", "home.toursSection.eyebrow")}
          {renderSection("home.excursionsSection.title", excursions, "/tours?type=excursion", "home.excursionsSection.eyebrow")}
          {renderSection("home.activitiesSection.title", activities, "/tours?type=activity", "home.activitiesSection.eyebrow")}
        </>
      )}
