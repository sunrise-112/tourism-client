...
// Services
import inclusionService from "../../services/inclusionsService";
...
const ManageInclusions = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await inclusionService.getAll(params);
      setInclusions(res?.data ?? []);
      setTotalItems(res?.pagination?.totalItems ?? 0);
    } catch {
      toast.error(t("manageInclusions.errors.fetchFailed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const translatedInclusions = inclusions?.map((inc) => ({
    ...inc,
    label: inclusionKeyMap[inc?.text]
      ? t(`manageInclusions.inclusions.${inclusionKeyMap[inc?.text]}`)
      : inc?.text,
  }));
  ...
  // ─── Selection ────────────────────────────────────────────
  const allSelected =
    inclusions.length > 0 && selectedIds.length === inclusions.length;

  const toggleSelectAll = () =>
    setSelectedIds(allSelected ? [] : inclusions.map((e) => e.id));
  ...
  // ─── Toggle active ────────────────────────────────────────
  const handleToggleActive = async (inclusion) => {
    const original = [...inclusions];
    try {
      setTogglingId(inclusion.id);
      await inclusionService.update(inclusion.id, {
        is_active: !inclusion.is_active,
      });
      setInclusions((prev) =>
        prev.map((e) =>
          e.id === inclusion.id ? { ...e, is_active: !e.is_active } : e
        )
      );
      toast.success(
        inclusion.is_active
          ? t("manageInclusions.toasts.deactivated")
          : t("manageInclusions.toasts.activated")
      );
    } catch (err) {
      setInclusions(original);
      toast.error(
        err?.response?.data?.message ||
          t("manageInclusions.errors.updateFailed")
      );
    } finally {
      setTogglingId(null);
    }
  ...
  const handleDeleteOne = async () => {
    try {
      setDeleting(true);
      await inclusionService.deleteOne(deleteModal.id);
      setInclusions((prev) => prev.filter((e) => e.id !== deleteModal.id));
      setSelectedIds((prev) => prev.filter((id) => id !== deleteModal.id));
      setTotalItems((n) => n - 1);
      toast.success(t("manageInclusions.toasts.deleteSuccess"));
      setDeleteModal(null);
    } catch (err) {
      toast.error(
        err?.response?.data?.message ||
          t("manageInclusions.errors.deleteFailed")
      );
    } finally {
      setDeleting(false);
    }
  ...
  const handleDeleteMany = async () => {
    try {
      ...
      await inclusionService.deleteMany(selectedIds);
      setInclusions((prev) => prev.filter((e) => !selectedIds.includes(e.id)));
      setTotalItems((n) => n - selectedIds.length);
      setSelectedIds([]);
      setBulkDeleteModal(false);
      toast.success(t("manageInclusions.toasts.bulkDeleteSuccess"));
    } catch (err) {
      toast.error(
        err?.response?.data?.message ||
          t("manageInclusions.errors.bulkDeleteFailed")
      );
    } finally {
  ...
  const openEdit = (inclusion) => {
    setFormValue({
      text: inclusion.text,
      is_active: inclusion.is_active,
    });
    setFormModal({ mode: "edit", data: inclusion });
  };

  const handleFormSave = async () => {
    if (!formValue.text.trim())
      return toast.error(t("manageInclusions.errors.nameRequired"));
    try {
      setFormSaving(true);
      if (formModal.mode === "create") {
        const created = await inclusionService.create(formValue);
        setInclusions((prev) => [created, ...prev]);
        setTotalItems((n) => n + 1);
        toast.success(t("manageInclusions.toasts.createSuccess"));
      } else {
        const updated = await inclusionService.update(
          formModal.data.id,
          formValue
        );
        setInclusions((prev) =>
        ...
        toast.success(t("manageInclusions.toasts.updateSuccess"));
      }
      setFormModal(null);
    } catch (err) {
      toast.error(
        err?.response?.data?.message || t("manageInclusions.errors.saveFailed")
      );
    } finally {
      setFormSaving(false);
    }
  ...
  // ─── Derived stats ────────────────────────────────────────
  const activeCount = inclusions.filter((e) => e.is_active).length;
  const inactiveCount = inclusions.filter((e) => !e.is_active).length;

  const getFilterLabel = (filterKey) => {
    switch (filterKey) {
      case "All":
        return t("manageInclusions.filters.all");
      case "Active":
        return t("manageInclusions.filters.active");
      case "Inactive":
        return t("manageInclusions.filters.inactive");
      default:
        return filterKey;
    }
  };
    ...
    >
      ...
      <div className='mb-8 flex items-start justify-between gap-4 flex-wrap'>
        <div>
          <p className='text-xs font-bold uppercase tracking-[0.2em] text-amber-500 mb-1'>
            {t("manageInclusions.admin")}
          </p>
          <h1
            className='text-3xl font-black text-stone-800'
            style={{ fontFamily: "'Playfair Display', serif" }}
          >
            {t("manageInclusions.title")}
          </h1>
          <p className='text-stone-400 text-sm mt-1'>
            {t("manageInclusions.totalInclusions", { count: totalItems })}
          </p>
        </div>
        <button
          onClick={openCreate}
        ...
        >
          ...
          {t("manageInclusions.newInclusion")}
        </button>
      </div>
      ...
      <div className='grid grid-cols-2 md:grid-cols-3 gap-4 mb-6'>
        {[
          {
            icon: "fa-gift",
            label: t("manageInclusions.stats.total"),
            value: totalItems,
          ...
          },
          {
            ...
            label: t("manageInclusions.stats.active"),
            value: activeCount,
          ...
          },
          {
            ...
            label: t("manageInclusions.stats.inactive"),
            value: inactiveCount,
      ...
      <div className='flex items-center justify-between gap-3 mb-5 flex-wrap'>
        ...
        {selectedIds.length > 0 && (
          ...
          >
            ...
            {t("manageInclusions.bulkDeleteButton", {
      ...
      {loading ? (
        ...
        <div className='bg-white rounded-2xl border border-stone-100 py-20 text-center'>
          <i className='fa fa-gift text-5xl text-stone-200 mb-4 block' />
          <p className='font-bold text-stone-500 mb-1'>
            {t("manageInclusions.empty.title")}
          </p>
          <p className='text-sm text-stone-400'>
            {t("manageInclusions.empty.description")}
          </p>
        </div>
      ) : (
            ...
            <table className='w-full text-sm'>
              <thead>
                <tr className='border-b border-stone-100 bg-stone-50'>
                  ...
                  {[
                    t("manageInclusions.table.serial"),
                    t("manageInclusions.table.name"),
                    t("manageInclusions.table.status"),
                    t("manageInclusions.table.created"),
                    t("manageInclusions.table.actions"),
                  ].map((h) => (
                    <th
                      key={h}
              ...
              <tbody className='divide-y divide-stone-50'>
                {translatedInclusions.map((e, idx) => (
                  <tr
                    key={e.id}
                  ...
                  >
                    ...
                    <td className='px-4 py-4'>
                      ...
                      >
                        ...
                        />
                        {e.is_active
                          ? t("manageInclusions.status.active")
                          : t("manageInclusions.status.inactive")}
                      </span>
                    </td>
                    ...
                    <td className='px-4 py-4'>
                      <div className='flex items-center gap-2'>
                        <button
                          ...
                          title={
                            e.is_active
                              ? t("manageInclusions.tooltips.deactivate")
                              : t("manageInclusions.tooltips.activate")
                          }
                        ...
                        <button
                          onClick={() => openEdit(e)}
                          title={t("manageInclusions.tooltips.edit")}
            ...
            <div className='p-6'>
              ...
              >
                {formModal.mode === "create"
                  ? t("manageInclusions.modal.createTitle")
                  : t("manageInclusions.modal.editTitle")}
              </h3>

              <label className='block mb-4'>
                <span className='text-xs font-bold uppercase tracking-wider text-stone-500 mb-1.5 block'>
                  {t("manageInclusions.modal.nameLabel")}
                </span>
                <input
                  type='text'
                  ...
                  placeholder={t("manageInclusions.modal.namePlaceholder")}
                  maxLength={300}
              ...
              <div className='flex gap-3'>
                ...
                >
                  {t("manageInclusions.modal.cancel")}
                </button>
                <button
                ...
                >
                  {formSaving ? (
                    <>
                      ...
                      {t("manageInclusions.modal.saving")}
                    </>
                  ) : formModal.mode === "create" ? (
                    t("manageInclusions.modal.create")
                  ) : (
                    t("manageInclusions.modal.saveChanges")
                  )}
                </button>
              </div>
            </div>
          ...
          <div className='bg-white rounded-2xl border border-stone-100 shadow-2xl w-full max-w-sm overflow-hidden'>
            ...
            <div className='p-6'>
              ...
              <h3
                ...
                style={{ fontFamily: "'Playfair Display', serif" }}
              >
                {t("manageInclusions.deleteModal.title")}
              </h3>
              <p className='text-sm text-stone-500 mb-6'>
                {t("manageInclusions.deleteModal.description", {
              ...
              <div className='flex gap-3'>
                <button
                  ...
                  className='flex-1 py-2.5 rounded-xl text-sm font-semibold text-stone-600 border border-stone-200 hover:bg-stone-50 transition-colors'
                >
                  {t("manageInclusions.deleteModal.cancel")}
                </button>
                <button
                ...
                >
                  {deleting ? (
                    <>
                      <i className='fa fa-spinner fa-spin mr-1.5' />
                      {t("manageInclusions.deleteModal.deleting")}
                    </>
                  ) : (
                    t("manageInclusions.deleteModal.delete")
                  )}
                </button>
              </div>
            </div>
          ...
          <div className='bg-white rounded-2xl border border-stone-100 shadow-2xl w-full max-w-sm overflow-hidden'>
            ...
            <div className='p-6'>
              ...
              <h3
                ...
                style={{ fontFamily: "'Playfair Display', serif" }}
              >
                {t("manageInclusions.bulkDeleteModal.title", {
                  count: selectedIds.length,
                })}
              </h3>
              <p className='text-sm text-stone-500 mb-6'>
                {t("manageInclusions.bulkDeleteModal.description")}
              </p>
              <div className='flex gap-3'>
                <button
                  ...
                  className='flex-1 py-2.5 rounded-xl text-sm font-semibold text-stone-600 border border-stone-200 hover:bg-stone-50 transition-colors'
                >
                  {t("manageInclusions.bulkDeleteModal.cancel")}
                </button>
                <button
                ...
                >
                  {bulkDeleting ? (
                    <>
                      <i className='fa fa-spinner fa-spin mr-1.5' />
                      {t("manageInclusions.bulkDeleteModal.deleting")}
                    </>
                  ) : (
                    t("manageInclusions.bulkDeleteModal.delete", {
                      count: selectedIds.length,
                    })
                  )}
                </button>
